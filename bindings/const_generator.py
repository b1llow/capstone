...
def is_with_prefix(x, prefix, target):
    ...
    return any(x.upper().startswith(pre.upper()) for pre in prefixs)
...
def gen(lang):
    ...
    for target in include:
        ...
        name = (prefix[0] if isinstance(prefix, list) else prefix).lower()
        outfile = open(
            templ["out_file"] % (name), "wb"
        )  # open as binary prevents windows newlines
        outfile.write((templ["header"] % (name)).encode("utf-8"))
        ...
        for line in lines:
            ...
            if not is_with_prefix(line, prefix, target):
                continue
            ...
            for t in tmp:
                ...
                if not is_with_prefix(f[0], prefix, target):
                    continue
                ...
                if "enum_header" in templ:
                    ...
                    if enum not in enums:
                        if len(enums) > 0:
                            write_enum_extra_options(
                                outfile, templ, last_enum, enums[last_enum]
                            )
                        ...
                        if "enum_doc" in templ:
                            for doc_line in doc_lines:
                                outfile.write(
                                    (templ["enum_doc"] % (doc_line)).encode("utf-8")
                                )
                            doc_lines = []
                    ...
                    if "option_sets" in templ and enum in templ["option_sets"]:
                        # option set format
                        line_format = templ["option_format"].format(
                            option="%s", type=enum, value="%s"
                        )
                        if value == 0:
                    ...
                    if "doc_line_format" in templ and "///<" in line:
                        ...
                        outfile.write(
                            (templ["doc_line_format"] % (doc)).encode("utf-8")
                        )
                else:
                    line_format = templ["line_format"]
